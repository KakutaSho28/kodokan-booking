...
const slotKey = computed(() => `slots-${props.date}-${props.therapistId || 'none'}`)
...
async function joinWaitlist(slot: AppointmentSlot) {
  ...
  try {
    const response = await $fetch<{ data: Waitlist, message: string }>(`${config.public.apiBase}/waitlists`, {
      method: 'POST',
...
<template>
  <div class="space-y-3">
    <div v-if="pending" class="flex items-center gap-2 text-sm font-medium text-[#2C5F8A]">
      <span class="size-4 animate-spin rounded-full border-2 border-blue-100 border-t-[#2C5F8A]" />
      空き状況を確認中
    </div>

    <div v-else-if="!enabled" class="rounded-lg border border-gray-200 bg-white p-4 text-sm text-gray-500">
      担当者と日付を選択してください。
    </div>
    ...
    <div v-else class="grid grid-cols-2 gap-3 lg:grid-cols-4">
      ...
      >
        ...
        >
          ...
          <p class="mt-1 text-xs" :class="selectedTime === slotTime(slot) ? 'text-blue-50' : 'text-gray-500'">
        ...
        <div v-else class="space-y-2">
          ...
          >
            <span v-if="readonly && (slot.waitlist_count || 0) > 0">× (待{{ slot.waitlist_count }}名)</span>
